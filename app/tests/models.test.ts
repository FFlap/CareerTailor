...
describe('model registry', () => {
  ...
  it('keeps OpenRouter wired up as a supported provider', () => {
    expect(DEFAULT_OPENROUTER_MODEL).toBe('arcee-ai/trinity-large-preview:free')
    expect(providerForModel(DEFAULT_OPENROUTER_MODEL)).toBe('openrouter')
    expect(isModelId(DEFAULT_OPENROUTER_MODEL)).toBe(true)
  })

  it('uses a vision-capable model for template-from-image', () => {
    expect(getModel(IMAGE_MODEL)?.vision).toBe(true)
  })
  ...
  it('normalizes stale stored model ids back to the default', () => {
    ...
    expect(normalizeModelId(DEFAULT_OPENROUTER_MODEL)).toBe(DEFAULT_OPENROUTER_MODEL)
  })
